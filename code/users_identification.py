...
"""
    users_identification: trains and validates the CNN for users identification
...
def cnn_encoder(x, parameters):
    ...
    z6 = tf.contrib.layers.fully_connected(p6, num_traces, activation_fn=None)
    return z6
...
def model(x_train, y_train, x_test, y_test, test_accuracy_old, num_epochs=100, minibatch_size=64, print_cost=True):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            prediction_train_label = np.argmax(prediction_train, axis=1) + 1
            prediction_test_label = np.argmax(prediction_test, axis=1) + 1

            if test_accuracy > test_accuracy_old:
                confusion_matrix_test = confusion_matrix_computation(num_traces, test_label, prediction_test_label)
                confusion_matrix_train = confusion_matrix_computation(num_traces, train_label, prediction_train_label)
                test_accuracy_old = test_accuracy

        gc.collect()
        np.save("../outputs/confusion_matrix_test.npy", confusion_matrix_test)
        np.save("../outputs/confusion_matrix_train.npy", confusion_matrix_train)
...
if __name__ == '__main__':
    ...
    index_vector = []
    user_new = 0
    for user in range(num_traces):
        print(user)
        with open('../processed_files/hiddens_' + 'train' + '_user' + str(user) + '.txt',
                  "rb") as fp:  # Unpickling
            sentences_u = pickle.load(fp)
        with open('../processed_files/times_' + 'train' + '_user' + str(user) + '.txt',
                  "rb") as fp:  # Unpickling
    ...
    output_matrix_tr = convert_to_one_hot(train_label, num_traces).T
    output_matrix_test = convert_to_one_hot(test_label, num_traces).T

    test_accuracy_input = 0
    _, test_accuracy_new, parameters_out, prediction_tr, prediction_tst = model(input_matrix_tr, output_matrix_tr,
                                                                                input_matrix_test, output_matrix_test,
                                                                                test_accuracy_input,
                                                                                num_epochs=num_epcs)
